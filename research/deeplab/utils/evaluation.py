...
def evaluate_repeatedly(checkpoint_dir,
  ...
  ##########################################################
  checkpoint_path_list = tf.train.get_checkpoint_state(checkpoint_dir).all_model_checkpoint_paths
...
#                   time.strftime('%Y-%m-%d-%H:%M:%S', time.gmtime()))
      logging.info('Starting evaluation on checkpoint ' + checkpoint_name)
      if eval_ops is not None:
        while not session.should_stop():
          session.run(eval_ops, feed_dict)

      logging.info('Finished evaluation on checkpoint  ' + checkpoint_name)
    num_evaluations += 1
